...
async function verifyOTP(req, res, next) {
  try {
    ...
    await prisma.session.create({
      data: { userId: user.id, refreshToken, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), userAgent: req.headers['user-agent'] || 'Unknown' },
    });
...
async function refreshToken(req, res, next) {
  try {
    ...
    let decoded;
    try { decoded = jwt.verify(rt, process.env.REFRESH_TOKEN_SECRET); }
    catch { return res.status(401).json({ success: false, error: 'Invalid or expired refresh token. Please sign in again.' }); }
    ...
    await prisma.session.update({
      where: { id: session.id },
      data: { refreshToken: newRefresh, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), userAgent: req.headers['user-agent'] || session.userAgent || 'Unknown' },
    });
...
async function resetPassword(req, res, next) {
  try {
    ...
    catch { return res.status(400).json({ success: false, error: 'Reset link is invalid or has expired. Please request a new one.' }); }
