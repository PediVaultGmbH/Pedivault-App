...
export function ResetPassword({ goTo, token }) {
  ...
  if (done) return (
    <div className="pv-si" style={{textAlign:'center'}}>
      ...
      <div className="pv-h">Password reset!</div>
      <div className="pv-s">Signing you in…</div>
    </div>
  );
  ...
  return (
    <div className="pv-si">
      <Brand/>
      <div className="pv-h-calm">Set new password</div>
      <div className="pv-s">Choose a strong password for your account</div>
      {err && <div className="pv-err">{err}</div>}
      <div className="pv-f">
        <label className="pv-lbl">New password</label>
        <div className="pv-iw">
          ...
          <input className="pv-in il ir" type={show?'text':'password'} value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="Min. 8 characters"/>
          <span className="pv-ir" onClick={()=>setShow(!show)}><Eye open={show}/></span>
        </div>
      </div>
      <div className="pv-f">
        <label className="pv-lbl">Confirm password</label>
        <div className="pv-iw">
          <span className="pv-il"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg></span>
          <input className="pv-in il ir" type={show2?'text':'password'} value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Repeat password"/>
      ...
      <button type="button" className="pv-btn" disabled={loading} onClick={handleReset}>
        {loading ? <><div className="pv-btn-spin"/>Resetting…</> : <>Set new password <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></>}
      </button>
      <div className="pv-sw">
        <span className="pv-lk" onClick={()=>goTo('signin')}>← Back to sign in</span>
      </div>
    </div>
  );
}
