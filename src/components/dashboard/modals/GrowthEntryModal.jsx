  ...
  return (
    <Modal open={open} onClose={handleClose} maxWidth={500}>
      <div className="pv-mhdr">
        ...
        <div className="pv-mhdr-eyebrow">{t('modals.growth_eyebrow','Growth Tracking')}</div>
        <div className="pv-mhdr-title">{t('modals.growth_title','Add Measurement')}</div>
        <div className="pv-mhdr-sub">{t('modals.growth_sub',"Record your child's latest growth measurements")}</div>
        <XBtn onClick={handleClose}/>
      </div>
      <div className="pv-mbody">
        <div className="fg">
          <label className="fl">{t('modals.dateMeasured','Date measured')} {errors.date&&<span style={{color:'#B82810',textTransform:'none',letterSpacing:0}}>— {errors.date}</span>}</label>
        ...
        <div className="fr" style={{marginTop:10}}>
          <div className="fg">
            <label className="fl">{t('modals.weightKg','Weight (kg)')}</label>
          ...
          <div className="fg"><label className="fl">{t('modals.heightCm','Height (cm)')}</label><input className="fi" type="number" step="0.1" placeholder="89" value={f.height} onChange={set('height')}/></div>
        </div>
        <div className="fr">
          <div className="fg"><label className="fl">{t('modals.headCirc','Head circ. (cm)')} <span>({t('modals.optional','opt.')})</span></label><input className="fi" type="number" step="0.1" placeholder="47" value={f.head} onChange={set('head')}/></div>
          <div className="fg"><label className="fl">{t('modals.recordedBy','Recorded by')}</label><input className="fi" placeholder="Dr. Kavita Singh" value={f.doctor} onChange={set('doctor')}/></div>
        </div>
      </div>
      <div className="pv-mfoot">
        ...
        <button type="button" className={`fb fb-p${loading?' fb-loading':''}`} onClick={submit}>
          {loading?<><div className="pv-spinner"/>&nbsp;{t('modals.saving','Saving…')}</>:<><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>{t('modals.saveMeasurement','Save Measurement')}</>}
        </button>
      </div>
    </Modal>
  );
