        ...
        {deleteStep===2 && (
          <div style={{padding:'32px 28px 24px'}}>
            ...
            <input value={deleteInput} onChange={e=>setDeleteInput(e.target.value.toUpperCase())} className="fi" placeholder="Type DELETE here"
              style={{width:'100%',boxSizing:'border-box',textAlign:'center',letterSpacing:'.15em',fontFamily:'monospace',fontSize:'.85rem',
                borderColor:deleteInput==='DELETE'?'var(--red)':'var(--line2)',
                background:deleteInput==='DELETE'?'rgba(185,40,20,.05)':'var(--cream-2)',transition:'all .2s'}}/>
            <div style={{marginTop:16,display:'flex',gap:10}}>
              ...
              <button type="button" disabled={deleteInput!=='DELETE'}
                ...
                style={{flex:1,height:40,borderRadius:10,border:'none',fontSize:'.6rem',fontWeight:500,
                  cursor:deleteInput==='DELETE'?'pointer':'default',transition:'all .2s',
      ...
      <Modal open={upgradeStep>0} onClose={()=>{if(upgradeStep!==3)setUpgradeStep(0);}} maxWidth={440}>
        ...
        {upgradeStep===2 && (
          <div style={{padding:'28px 28px 24px'}}>
            ...
            <div style={{marginBottom:12}}>
              ...
              <input className="fi" value={card.number} onChange={e=>setCard(c=>({...c,number:e.target.value.replace(/\D/g,'').slice(0,16).replace(/(.{4})/g,'$1 ').trim()}))} placeholder="1234 5678 9012 3456" style={{width:'100%',boxSizing:'border-box',fontFamily:'monospace',letterSpacing:'.08em'}}/>
            </div>
            <div style={{marginBottom:12}}>
            ...
            <button type="button" disabled={!card.number||!card.name||!card.expiry||!card.cvv||paying}
              onClick={()=>{if(!card.number||!card.name||!card.expiry||!card.cvv)return;setPaying(true);setTimeout(()=>{setPaying(false);setUpgradeStep(3);},1800);}}
              style={{width:'100%',height:44,marginTop:16,borderRadius:12,border:'none',fontSize:'.66rem',fontWeight:600,cursor:'pointer',transition:'all .2s',display:'flex',alignItems:'center',justifyContent:'center',gap:8,
        ...
        {upgradeStep===3 && (
          <div style={{padding:'48px 28px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',gap:14,background:'linear-gradient(180deg,var(--rose-pale),var(--white))'}}>
            ...
            <button type="button" onClick={()=>{setUpgradeStep(0);setCard({number:'',name:'',expiry:'',cvv:'',});}} style={{height:40,padding:'0 28px',borderRadius:12,border:'none',background:'var(--rose)',color:'#fff',fontSize:'.62rem',fontWeight:600,cursor:'pointer',boxShadow:'0 4px 16px rgba(155,58,86,.28)',marginTop:4}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <div className="card">
            ...
            <div style={{marginTop:12}}>
              ...
              <input className="fi" type="email" value={profile.email} disabled style={{width:'100%',boxSizing:'border-box',opacity:.6,cursor:'not-allowed'}} placeholder="your@email.de"/>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <div className="card">
            ...
            <div style={{display:'flex',justifyContent:'flex-end',marginTop:4}}>
              <button type="button" onClick={handleChangePassword} disabled={pwdSaving||!pwd.current||!pwd.next||pwd.next!==pwd.confirm} style={{
                height:36,padding:'0 20px',borderRadius:10,border:'none',fontSize:'.6rem',fontWeight:500,cursor:'pointer',transition:'all .2s',
                ...
                opacity:pwdSaving?.6:1,
              }}>{pwdSaving ? 'Updating…' : 'Update password'}</button>
            </div>
          </div>
          ...
          <div className="card">
            ...
            {sessionsLoading ? (
              <div style={{fontSize:'.56rem',color:'var(--ink-3)',padding:'12px 0'}}>Loading sessions…</div>
            ) : sessions.length === 0 ? (
              <div style={{fontSize:'.56rem',color:'var(--ink-3)',padding:'12px 0'}}>No active sessions found.</div>
            ) : sessions.map((s,i)=>(
